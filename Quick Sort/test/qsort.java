...
public class qsort
{ 
    public static void OutputFile(float[] OutputList,long duration,int filenumber,String filename) throws IOException 
	{
			...
			bw.newLine();
			bw.write("Input file		Size		Sorting Time(in milliseconds)");
			bw.newLine();
			bw.write(filename+"		"OutputList.length+"		"+duration);
			bw.newLine();
			bw.newLine();
		...
		else
		{
			for(int filenumber=0;filenumber<args.length;filenumber++)
			{
				...
				{					
					...
					OutputFile(Inputlist,time2-time1,filenumber+1,filename);
